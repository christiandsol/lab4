...
void write_block_group_descriptor_table(int fd) {
	...
	block_group_descriptor.bg_used_dirs_count = 2; /* Lost+Found and rootdir*/ 
...
void write_block_bitmap(int fd)
{
    ...
    for (int i = 0; i < (LAST_BLOCK + 1) >> 3;  i++) {
        map_value[i] = i != 2 ? 0xFF  : 0x7F;
    }

    //bitmap padding
    for (int i = (NUM_BLOCKS >> 3) - 1; i < (BLOCK_SIZE); i++) {
            map_value[i] =  0xFF;
    }
...
void write_inode_bitmap(int fd)
{
    ...
    for (int i = 0; i < (LAST_INO >> 3) + 1;  i++) {
        map_value[i] = i != 1 ? 0xFF : 0x1F;
    }
